...
struct sdhc_semmc_data {
	...
	// uint32_t           response[NRF_SEMMC_RESPONSE_SIZE + 2];
};
			...
			struct sdhc_data *data)
{
	...
	nrf_semmc_cmd_desc_t semmc_cmd = {
		...
		.arg = cmd->arg,
		// .resp_buffer = (void *)0x2f890000,
		// .resp_buffer = dev_data->response,
		.resp_buffer = (uint8_t *)dev_config->semmc.p_reg + 0x200,
	};
	nrf_semmc_transfer_desc_t semmc_transfer = {
		.buffer = NULL,
	};
	...
	if (cmd->opcode == SD_READ_MULTIPLE_BLOCK ||
		...
		rc = send_request(dev, &blk_cnt_cmd, &semmc_transfer, K_MSEC(1));
		if (rc < 0) {
			return rc;
		}
	}
	...
	if (data) {
		...
		semmc_transfer.block_size = data->block_size;
		semmc_transfer.num_blocks = data->blocks;
	}

	rc = send_request(dev, &semmc_cmd, &semmc_transfer, K_MSEC(cmd->timeout_ms));
...
static int api_request(const struct device *dev,
		       ...
		       struct sdhc_data *data)
{
	// LOG_INF("api_request - CMD%u, timeout: %u ms, retries: %u",
	// 	cmd->opcode, cmd->timeout_ms, cmd->retries);
